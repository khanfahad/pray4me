...
function RequestList({ requests, isAtHolySite, onMakeDua, duaJustMade, sortMode, setSortMode, activeCategory, setActiveCategory, onReshuffle }) {
...
export default function MakeDuaPage({ locationState }) {
  ...
  // All requests (for holy site view)
  const allFiltered = useMemo(() => {
    const base = sortMode === 'shuffle' ? shuffled : requests;
    return applySort(applyCategory(base));
  }, [requests, sortMode, activeCategory, shuffled]);

  // Only requests that allow non-holy-site duas
  const anywhereFiltered = useMemo(() => {
    const base = (sortMode === 'shuffle' ? shuffled : requests).filter(r => !r.holyOnly);
    return applySort(applyCategory(base));
  }, [requests, sortMode, activeCategory, shuffled]);
  ...
  const handleMakeDua = async (request, fromHolySite) => {
    ...
    try {
      const locationId = fromHolySite ? locationState.currentSite?.id : null;
      await FirebaseService.markDuaAsMade(request.id, user.id, user.displayName, locationId);
      setDuaJustMade(request.id);
      await refreshUser();
      const msg = fromHolySite
        ? 'Ameen! A blessed dua from a holy site.'
        : 'Ameen! May Allah accept your dua from wherever you are.';
    ...
    <div className="page-content">
      ...
      {isAtHolySite ? (
          ...
          <div className="container" style={{ marginTop: 18 }}>
            <div className="info-banner green">
              ...
              <div className="banner-text">
                ...
                Your duas here carry special blessing. Tap the button below each request after you've prayed for them.
              </div>
            </div>
        ...
        <div style={{ marginTop: 18 }}>
          {/* Make Dua from Anywhere section */}
          <div className="container">
